...
@Singleton
class MainRepository @Inject constructor(private val apiRetrofit: ApiRetrofit) : BaseRepository() {

    suspend fun checkSMS(param1: String, param2: String) =
        apiRetrofit.onCheckSMS(param1, param2)

    suspend fun boxOfficeApi(targetDt: String): ApiResult<Result> {
        return getResponse(apiRetrofit.getBoxOffice(Constants.API_KEY, targetDt))
    }
